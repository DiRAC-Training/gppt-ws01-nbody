...
__global__ void calc_acc_tiled(Vec2 *acc, const Vec2 *pos, const real *mass,
  ...
  const int gtid = blockIdx.x * blockDim.x + threadIdx.x;
  // Guard with ternary as threads past the end cannot return early - they must
  // still reach every __syncthreads() and help load tiles for the rest of the
  // block. Using padding instead also avoids branching.
  const Vec2 pi = (gtid < N) ? pos[gtid] : Vec2{0.0, 0.0};
  Vec2 accl = {0.0, 0.0};

  for (int tile_start = 0; tile_start < N; tile_start += THREADS_PER_BLOCK) {
...
int main() {
  ...
  const int n_blocks =
      (N_PARTICLES + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
  ...
  calc_acc_tiled<<<n_blocks, THREADS_PER_BLOCK>>>(
      acc_d.data().get(), pos_d.data().get(), mass_d.data().get(), N_PARTICLES,
      epsilon);
  ...
  while (t < total_time - 0.5 * dt) {
    calc_acc_tiled<<<n_blocks, THREADS_PER_BLOCK>>>(
        acc_d.data().get(), pos_d.data().get(), mass_d.data().get(),
        N_PARTICLES, epsilon);
    advance_pos<<<n_blocks, THREADS_PER_BLOCK>>>(
        pos_prev_d.data().get(), pos_d.data().get(), acc_d.data().get(),
        N_PARTICLES, dt);
...
bool test_calc_acc_x() {
  ...
  calc_acc_tiled<<<1, THREADS_PER_BLOCK>>>(acc.data().get(), pos.data().get(),
                                           mass.data().get(), pos.size(),
                                           epsilon);
  cudaDeviceSynchronize();
