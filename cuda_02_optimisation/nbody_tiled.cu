...
__global__ void calc_acc_tiled(Vec2 *acc, const Vec2 *pos, const real *mass,
  ...
  __shared__ real shMass[block_size];
...
int main(int argc, char *argv[]) {
  ...
  const uint block_size = get_argval<uint>(argv, argv + argc, "--bs", 32);
  ...
  // Setup initial conditions
  {
    ...
    calc_acc_tiled<<<n_blocks, block_size>>>(
...
bool test_calc_acc_x() {
  ...
  calc_acc_tiled<<<1, 32>>>(acc.data().get(), pos.data().get(),
                            mass.data().get(), pos.size(), epsilon);
  cudaDeviceSynchronize();
...
bool test_advance_pos() {
  ...
  advance_pos<<<1, 32>>>(pos_prev.data().get(), pos.data().get(),
